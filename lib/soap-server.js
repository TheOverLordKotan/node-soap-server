...
SoapOperation.prototype.getType = function(options){
	console.log('Kot 1061  options '+ typeOfD(options));
	if(options === null){
		console.log('Kot 999  options');
		options = 'string';
	}
	console.log('Kot 1062  string '+ ('options ' == typeof options));
	console.log('Kot 1061  options '+ typeOfD(options));
	if('string' == typeof options){
		console.log('Kot 1066  string');
		options = {type: options};
	}
	console.log('Kot 1063  function '+ ('object' == typeof options));
	console.log('Kot 1061  options '+ typeOfD(options));
	if('function' == typeof options){
		console.log('Kot 106  function');
...
function typeOfD(obj) {
  return {}.toString.call(obj).split(' ')[1].slice(0, -1).toLowerCase();
}
SoapOperation.prototype.setOutputType = function(name, options){
		console.log('setOutputType : '+name);
		console.log('setOutputType : '+options);
...
SoapOperation.prototype.setInputType = function(name, options){
	console.log('setInputType : '+name);
		console.log('setInputType : '+options);
	options = this.getType(options);
	this.inputs[name] = options;
};
...
function SoapServer(options){
	//console.log('SoapServer :'+options);
	if(!options){
		options = {
			tns: 'server.soap.com'
		};
...
for (var property in service) {
  output += property + ': ' + service[property]+'; ';
}
//console.log(output);
...
SoapServer.prototype.castArgument = function(inputName, requestArgument, options){
	//console.log('castArgument :'+inputName);
	switch(options.type){
		case 'object':
...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
	var operationRequestFirs = soapRequest['soapenv:Envelope']['soapenv:Body'][0];
	var operationRequest = deepClone(operationRequestFirs);
	
	for(var operationRequestName in operationRequest){
		var operationName10 = operationRequestName.substr(operationRequestName.indexOf(':') + 1);
		var operationName = operationName10.replace("entrada", "");
		...
		for(var inputName in operation.inputs){
			...
			var requestArgument = this.castArgument(inputName, requestArguments['elementoEntrada'], input);
		...
		*/
		...
		var response = {};
		var responseName = 'salida' +operationName ;
		response[responseName] = {'elementoSalida': operationResponse};
		return response;
	}
...
function deepClone(obj) {
    ...
    if (typeof obj !== "object") { 
		...
		var result = obj.replace(prefixMatch, '');
        r = result;
    }
    ...
    // Object
        ...
        for (i in obj) {
			var prefixMatch = new RegExp(/(?!xmlns)^.*:/);
			var ii = i.replace(prefixMatch, '');
            r[ii] = deepClone(obj[i]);
        }
    }
	...
	if (req.method == 'POST') {
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				responseXml += '	xmlns:dpi="http://www.itosmosys.com/datapi/servicios/' + serviceName + '" xmlns:xscript="http://apache.org/xsp/xscript/1.0" xmlns:osm-utils="http://osmosyscol.com/logicsheets/osm-utils/1.0" xmlns:util="http://apache.org/xsp/util/2.0" xmlns:cinclude="http://apache.org/cocoon/include/1.0" xmlns:xsp-request="http://apache.org/xsp/request/2.0">\n';
				responseXml += '	<soap:Body>\n';
				
				try{
					var soapResponse = This.handleOperationRequest(service, soapRequest);
					console.dir(soapResponse);
					responseXml += This.js2xml(soapResponse, 'dpi', 2);
				}
				catch(exception){
					console.dir('Request parse error 100 : ' + exception);
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	wsdl += '	xmlns:dpi="http://www.itosmosys.com/datapi/servicios/' + service.name + '/"\n';
	wsdl += '	targetNamespace="http://www.itosmosys.com/datapi/servicios/' + service.name+ '/"\n';
	wsdl += '	name="' + service.name + 'Service">\n';
	
	wsdl += '	<types>\n';
	wsdl += '		<xsd:schema version="1.0" elementFormDefault="qualified" targetNamespace="http://www.itosmosys.com/datapi/servicios/' + service.name+ '/">\n';
	wsdl += '		<xsd:element type="dpi:tipoEntrada'+ service.name+'" name="entrada'+ service.name+'" />\n';
	wsdl += '		<xsd:element type="dpi:tipoSalida'+ service.name+'" name="salida'+ service.name+'" />\n';
	
	wsdl += '		    <xsd:complexType name="tipoEntrada'+ service.name+'">\n';
    wsdl += '		        <xsd:sequence>\n';
    wsdl += '		           <xsd:element type="dpi:tipoElementoEntrada'+ service.name+'" name="elementoEntrada" minOccurs="1" maxOccurs="1" />\n';
    wsdl += '		        </xsd:sequence>\n';
    wsdl += '		     </xsd:complexType>\n';
    wsdl += '		     <xsd:complexType name="tipoSalida'+ service.name+'">\n';
    wsdl += '		        <xsd:sequence>\n';
    wsdl += '		           <xsd:element type="dpi:tipoElementoSalida'+ service.name+'" name="elementoSalida" minOccurs="0" maxOccurs="unbounded" />\n';
    wsdl += '		        </xsd:sequence>\n';
    wsdl += '		     </xsd:complexType>\n';
	...
	for(operationName in service.operations){
		...
		wsdl += '	</message>\n';
		*/
		wsdl += '	<message name="'+ service.name+'Request"> \n';
		wsdl += '		<part name="entrada" element="dpi:entrada'+ service.name+'" /> \n';
		wsdl += '	</message>\n';
		wsdl += '	<message name="'+ service.name+'Response">\n';
		wsdl += '		<part name="salida" element="dpi:salida'+ service.name+'" />\n';
		wsdl += '	</message>\n';
	...
	for(operationName in service.operations){
		...
		wsdl += '			<input message="dpi:'+ service.name+'Request" />\n';
		wsdl += '			<output message="dpi:'+ service.name+'Response" />\n';
		wsdl += '		</operation>\n';
	}
	wsdl += '	</portType>\n';

	wsdl += '	<binding type="dpi:' + operationName + '" name="' + operationName + 'SOAP">\n';
	...
	*/
    ...
    wsdl += '     <soap:operation soapAction="http://www.itosmosys.com/datapi/servicios/' + service.name + '" />\n';
    wsdl += '     <input>\n';
    wsdl += '        <soap:body use="literal" />\n';
    wsdl += '     </input>\n';
    wsdl += '     <output>\n';
	...
	wsdl += '		<port name="' + service.name + 'SOAP" binding="dpi:' + service.name + 'SOAP">\n';
